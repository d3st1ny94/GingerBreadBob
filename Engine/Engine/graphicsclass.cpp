...
bool GraphicsClass::Initialize(int screenWidth, int screenHeight, HWND hwnd)
{
	...
	result = m_GameOver->Initialize(m_D3D->GetDevice(), screenWidth, screenHeight, L"../Engine/data/GameOver.dds", screenWidth / 4, screenHeight / 4);
	if (!result)
	{
		MessageBox(hwnd, L"Could not initialize the bitmap object.", L"Error", MB_OK);
		return false;
...
bool GraphicsClass::Frame(float deltatime, std::vector<std::vector<int>>& toDraw, int score, bool gameover)
{
	bool result;

	CurrTileX += 40 * deltatime;
	// Render the graphics scene.
	result = Render(deltatime, toDraw, score, gameover);
	if(!result)
	{
		return false;
	}
...
bool GraphicsClass::Render(float deltatime, std::vector<std::vector<int>>& toDraw, int score, bool gameover)
{
	...
	{
		//prepare gameover bitmap for drawing
		...
		result = m_SimpleTexShader->Render(m_D3D->GetDeviceContext(), 6, S, viewMatrix, orthoMatrix, m_GameOver->GetTexture());
		if (!result)
		{
			return false;
		}
