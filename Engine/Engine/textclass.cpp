						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence1, "Score:", 100, 100, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
	...
	// Now update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, "0", 100, 200, 1.0f, 0.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
bool TextClass::UpdateScore(int score, ID3D11DeviceContext* deviceContext){
	stringstream ss;
	string sScore;
	ss.str(sScore);
	ss << score;
	sScore = ss.str();
	return UpdateSentence(m_sentence2, sScore.c_str(), 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
}
